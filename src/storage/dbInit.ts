      ...
      if (currentVersion < TARGET_VERSION) {
        ...
        if (currentVersion < 3) {
          ...
          if (tableExists('time_blocks')) {
            try {
              (tx as any).executeSync('ALTER TABLE time_blocks ADD COLUMN recurrence_rule TEXT');
            } catch (e: unknown) {
              const msg = e instanceof Error ? e.message : String(e);
              if (!msg.toLowerCase().includes('duplicate column')) {
                throw e;
          ...
          if (tableExists('tasks')) {
            try {
              (tx as any).executeSync('ALTER TABLE tasks ADD COLUMN recurrence_rule TEXT');
            } catch (e: unknown) {
              const msg = e instanceof Error ? e.message : String(e);
              if (!msg.toLowerCase().includes('duplicate column')) {
                throw e;
          ...
          if (tableExists('events')) {
            try {
              (tx as any).executeSync('ALTER TABLE events ADD COLUMN recurrence_rule TEXT');
            } catch (e: unknown) {
              const msg = e instanceof Error ? e.message : String(e);
              if (!msg.toLowerCase().includes('duplicate column')) {
                throw e;
